# What’s required to put together a Watchmen Framework.
------
...
# <img src="src/docs/feature-file.png" width="400" height="200"> 

**Steps definition**


 - The glue between Gherkin and the system under test are implemented as regular Java methods and implemented in regular Java class "CommonApiStepsDefinition.java" and "AwsStepsDefinition.java".

 - Later on project it may be a good idea to divide steps between few Java classes . It is, however, not needed for today's state of a project.
...
**Managing Dependency**
 ...
 - Class TestScope.java holding the state of scenario. This is a singleton and to create and managing instances of this class we will use Spring library. Spring will do all dependency managing for us, including making sure instance ends up where we need it. 

 - To configure Spring we need a class called <SpringConfig>.java  in src/main/java/com/ally/d3/watchmen/config. 

 - To avoid leakage between scenarios we have to reset TestScope on "Before" hook: 


**Execute tests**

 - To run Cucumber feature files framework uses JUnit test runner.

 - To configure JUnit runner we need a file called <featureName>.java  in src/main/java/com/ally/d3/watchmen/testRunner/
...
The @RunWith (Cucumber.class) annotation on the TestRunner class tells JUnit to kick off Cucumber. Cucumber run time parses the command-line options to know what Feature to run, where the Glue Code lives, what plugins to use, and so on.


This above configurations telling Cucumber next things:
...
**Project structure**
 ...
 - To configure Spring to use property file we need to update class called <SpringConfig>.java  in src/main/java/com/ally/d3/watchmen/config: 
...
If compile was successful - your are ready to start testing your API.



Create your directory (name of directory - your api name) under src/main/resource/feature.
Create your feature.file
Create your Scenarios reusing existing steps (Steps Definition)
Update config.properties file with your id and password (needed for proxy authentication) , under src/main/resources.
Create your  Run_your_feature class using template under src/main/java/testRunner.
Right click on Run_your_feature.java and Execute it


How to run from command line

change directory to your project directory (that folder where pom.xml is) (use cd command for that)
to make sure your project is fine and all dependencies are on place - build it (you can skip this step if your project has not changed recently):  mvn clean install -Dmaven.wagon.http.ssl.insecure=true -Dmaven.wagon.http.ssl.allowall=true -Dmaven.wagon.http.ssl.ignore.validity.dates=true
Run your TestRun.java (had been created on step 5 above): mvn test  -Dtest=TestRun
